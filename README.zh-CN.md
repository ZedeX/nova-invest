...
| **LLM 路由** | OpenAI / Anthropic / Mock | 成本上限 + 重试 + 降级 |
| **测试** | Vitest + Playwright | 284 单元/集成 + 9 e2e |
...
| **实现**：[web/src/lib/ask/citation.ts](./web/src/lib/ask/citation.ts)
...
Nova Invest 是 **测试优先** 构建的。每份 ADR 在 [docs/tdd/](./docs/tdd/)
都有对应的 TDD 规格，每个代码模块都有同目录单元测试。

| 测试层 | 数量 | 运行器 | 覆盖范围 |
|--------|------|--------|---------|
| **单元** | 263 | Vitest | 全部 16 个 lib 模块 + ADR 合规 |
| **集成** | 12 | Vitest | Agent Loop + RAG 管线端到端 |
| **E2E** | 9 | Playwright | 跨 Epic 用户旅程 |
| **总计** | **284 + 9** | — | — |

```bash
pnpm test              # 运行单元 + 集成
pnpm test:watch        # 监听模式
pnpm test:coverage     # 带 V8 覆盖率
pnpm test:e2e          # 运行 Playwright e2e（需要先 `pnpm dev`）
pnpm test:all          # 全部
```

📖 **TDD 文档**：[docs/tdd/](./docs/tdd/)（5 份：策略、单元、集成、e2e、fixtures、覆盖率矩阵）
...
| 🧪 **TDD** | [docs/tdd/](./docs/tdd/) | 测试策略 + 覆盖率矩阵 |
...
3. [TDD 策略](./docs/tdd/00-test-strategy.md) — 测试先行
4. 提交 PR 前运行 `pnpm test`——284 个测试必须全过

### 代码风格

- **TypeScript** 严格模式（业务逻辑 0 `any`，0 `unknown` 强转）
- **ESLint** + **Prettier** 强制（0 错误，3 个历史警告）
